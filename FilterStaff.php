...
if($_POST['did'] == '')
{$filter[] = " (department.name LIKE '%' or department.name is NULL) ";}
else
{$filter[] = " department.name = " . $sq . $_POST['did'] . $sq;}

$query = "SELECT hospital_staff.id, fname, lname, title, hire, department.name FROM (hospital_staff INNER JOIN department ON dept = department.id)";
...
if(!$stmt->bind_result($sid, $fname, $lname, $title, $hire, $dept)){
echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
}
while($stmt->fetch()){
echo "<tr><td>" . $sid . "</td><td>" . $fname . "</td><td>" . $lname . "</td><td>" . $title . "</td><td>"
. $hire . "</td><td>" . $dept . "</td></tr>";
}
$stmt->close();
?>
	</table>
...
<div>
  <a href="http://web.engr.oregonstate.edu/~mcinnisb/front_desk/SearchStaff.php">Back</a>
  </div>
</body>
</html>
<?php
...
//Connects to the database
$servername = "oniddb.cws.oregonstate.edu";
$username = "mcinnisb-db";
$password = "4CW9sgnptnGex5pb";
$dbname = "mcinnisb-db";
